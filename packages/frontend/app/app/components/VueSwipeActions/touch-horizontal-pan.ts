...
export type Pan = ReturnType<typeof processChanges>;
...
export interface Context {
  handler: (changes: Pan) => void;
  direction: Direction;
  mouseStart: (evt: MouseEvent) => void;
  mouseEnd: (evt: MouseEvent) => void;
  move: (evt: MouseEvent) => void;
  start: (evt: TouchEvent | MouseEvent, mouseEvent: boolean) => void;
  end: (evt: TouchEvent | MouseEvent) => void;
...
function getDirection(mod: Direction) {
...
function processChanges(
  ...
  let direction;
  const distX = pos.left - (ctx.event?.x || 0);
  const distY = pos.top - (ctx.event?.y || 0);
  ...
  return {
    ...
    duration: new Date().getTime() - (ctx.event?.time || 0),
    distance: {
      x: absDistX,
      y: absDistY
    },
    ...
    delta: {
      x: pos.left - (ctx.event?.lastX || 0),
      y: pos.top - (ctx.event?.lastY || 0)
    }
  };
}

function shouldTrigger(ctx: Context, changes: Pan) {
...
export default {
  ...
  beforeMount(el, binding) {
    ...
    function handleEvent(evt: Event, mouseEvent: boolean) {
      if (mouse && mouseEvent) {
    ...
    if (el.__qtouchpan) el.__qtouchpan_old = el.__qtouchpan;
  ...
  updated(el, { oldValue, value, modifiers }) {
    ...
    if (oldValue !== value) ctx.handler = value;
